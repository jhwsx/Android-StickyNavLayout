	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mInnerScrollView instanceof ScrollView) {
				...
				if (!isInControl && androidx.core.view.ViewCompat.canScrollVertically(rv, -1) && isTopHidden
						&& dy > 0) {
					isInControl = true;
					ev.setAction(MotionEvent.ACTION_CANCEL);
					MotionEvent ev2 = MotionEvent.obtain(ev);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (Math.abs(dy) > mTouchSlop) {
				...
				if (mInnerScrollView instanceof ScrollView) {
					...
					RecyclerView rv = (RecyclerView) mInnerScrollView;
					if (!isTopHidden || (!androidx.core.view.ViewCompat.canScrollVertically(rv, -1) && isTopHidden && dy > 0)) {
						initVelocityTrackerIfNotExists();
						mVelocityTracker.addMovement(ev);
						mLastY = y;
						return true;
